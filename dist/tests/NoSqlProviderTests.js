...
const SqlProviderBase_1 = require("../SqlProviderBase");
                ...
                if (provName.indexOf('sql') !== -1) {
                    it('MultiEntry multipart index - large index put', () => {
                        return openProvider(provName, {
                            ...
                            const keys = [];
                            lodash_1.times(1000, () => {
                                keys.push(lodash_1.uniqueId('multipartKey'));
                            });
                    ...
                    function testBatchUpgrade(expectedCallCount, itemByteSize) {
                        ...
                        return openProvider(provName, {
                            ...
                            if (provName.indexOf('sql') !== -1) {
                                ...
                                transactionSpy = sinon_1.spy(SqlProviderBase_1.SqlTransaction.prototype, 'internal_getResultsFromQuery');
                            }
                            return openProvider(provName, {
                                version: 2,
                                stores: [
                    ...
                    if (provName.indexOf('indexeddb') !== 0) {
                        ...
                        it('Removes index without pulling data to JS', () => {
                            ...
                            return openProvider(provName, {
                                ...
                                storeSpy = sinon_1.spy(SqlProviderBase_1.SqlTransaction.prototype, 'getStore');
                                return openProvider(provName, {
                                    version: 2,
                                    stores: [
                                        {
                                    ...
                                    sinon_1.assert.notCalled(storeSpy);
                                ...
                                return openProvider(provName, {
                                    ...
                                    const p2 = prov.openTransaction(undefined, false).then(trans => {
                                        return trans
                                            ...
                                            .then(fullMeta => {
                                            const oldIndexReallyExists = lodash_1.some(fullMeta, (metaObj) => {
                        ...
                        it('Add remove index in the same upgrade, while preserving other indices', () => {
                            return openProvider(provName, {
                                ...
                                return openProvider(provName, {
                                    ...
                                    const p2 = prov.getOnly('test', 'ind2', 'aa').then((items) => {
                                        assert.equal(items.length, 1);
                                        assert.equal(items[0].id, 'abc');
                                        assert.equal(items[0].tt, 'a');
                                        assert.equal(items[0].zz, 'aa');
