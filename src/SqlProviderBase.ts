...
export abstract class SqliteSqlTransaction extends SqlTransaction {
    private _pendingQueries: IDeferred<any>[] = [];
...
class SqlStore implements DbStore {
    ...
    put(itemOrItems: ItemType | ItemType[]): Promise<void> {
        ...
        if (isError(datas)) {
            return Promise.reject<void>(datas);
        }
        ...
        if (some(this._schema.indexes, index => indexUsesSeparateTable(index, this._supportsFTS3))) {
            ...
            // Track 0-based index of item we're looking at
            let itemIndex = -1;
            for (const item of items) {
                ...
                if (this._schema.indexes) {
                    // Track 0-based index of index we're looking at
                    let indexIndex = -1;
                    for (const index of this._schema.indexes) {
                        indexIndex++;
                        ...
                        if (serializedKeys.length > 0) {
                            ...
                            for (const val of serializedKeys) {
